...
//               wait.until(expectation);
	...
	public void Click(By element, String nameofelement){	
		...
		try{
			...
			report.updateTestLog("Clicking "+nameofelement, nameofelement+" is clicked", Status.DONE);
		}catch(Exception e){
			System.out.println(nameofelement+" is not clicked");
			report.updateTestLog("Check_ClickElement ", nameofelement+" is not clicked"+e.toString(), Status.FAIL);
		}
		
	}
	
public void ClickElement (By element, String nameofelement){	
	...
	public void Javascript_Click_Scroll( By element,String Operation) {
		try{
			...
			System.out.println("Javascript_Click_Scroll  error "+e);
			report.updateTestLog("Javascript_Click_Scroll ", "Javascript_Click_Scroll  error "+e.toString(), Status.FAIL);
		}
	...
	public void SendKeys(By element, String value) {
		try{
			...
			System.out.println(value+"Edit box is not displayed");
			report.updateTestLog("Fill_Editbox", element+"Edit box is not displyed"+e.toString(), Status.FAIL);
		}
...
public boolean isDisplayed_boolean(By element, String element_name){
	try{
			if (driver.findElement(element).isDisplayed()) {
			report.updateTestLog("Element Displayed " + element_name, element_name+" displayed successfully", Status.PASS);
...
public boolean isDisplayed_boolean_special(String element, String element_name){
	...
	public void Mouseover(By element){
		...
		try{
			...
			report.updateTestLog("Mouse over the element", "Mouseover is successful", Status.DONE);
			System.out.println("Mouseover is successful");
		}catch(Exception e){
			report.updateTestLog("Mouseover", element+"element is not displayed"+e.toString(), Status.FAIL);
		}
	...
	public void SelectDropdown(By element, String value, String Type){
		try{
			...
			report.updateTestLog("SelectDropdown"+Type, element+" Select box is nt dsiplayed "+e.toString(), Status.FAIL);
		}
	...
	public void ElementExist(By element, String value){
		try{	
			...
			report.updateTestLog("ElementExist", value+" Element is not displyed"+e.toString(), Status.FAIL);
		}
	}
	...
	try{	
		...
		Javascript_Click_Scroll(element, "Scroll");
		if (driver.findElement(element).isDisplayed()) {
			report.updateTestLog("Displaying "+nameofelement, nameofelement+" exists", Status.DONE);
			System.out.println(nameofelement+" exists");
		}else{
			report.updateTestLog("Displaying "+nameofelement, nameofelement+" does not exists", Status.FAIL);			
			System.out.println(nameofelement+" does not exists");
		}
		
		if (driver.findElement(element).getText().equalsIgnoreCase(valueofelement)) {
			report.updateTestLog("Checking the data", "Data found : "+valueofelement , Status.DONE);
			System.out.println("actual:"+driver.findElement(element).getText()+"expected:"+valueofelement+"are same");
		}else{
			report.updateTestLog("Checking the data", "Data not found : "+valueofelement , Status.FAIL);
			System.out.println("actual:"+driver.findElement(element).getText()+"expected:"+valueofelement+"are not same");
		}
	}catch(Exception e){
		report.updateTestLog("Check_RetrieveValue", nameofelement+"Element is not displayed "+e.toString(), Status.FAIL);
	}
	}
	public void Javascript_Click_Scroll_WebElement( WebElement element,String Operation) {
		try{
			...
			System.out.println("Javascript_Click_Scroll  error "+e);
			report.updateTestLog("Javascript_Click_Scroll_WebElement", "Javascript_Click_Scroll  error "+e.toString(), Status.FAIL);
		}
...
//			report.updateTestLog("Element validation",Element_Name+ " element is not Displayed", Status.FAIL);
	...
	public String window_handle(){
		...
		try{
			...
			report.updateTestLog("Get window Handle", "WIndow handle exception "+e.toString(), Status.FAIL);
			return parentwindow;
		}
	}
...
public void Fill_Editbox(By element, String value) {
	...
	report.updateTestLog("Entering data in editbox", value+"is entered in the "+nameofelement, Status.DONE);
...
public void Check_Page(By element, String pagename){
	...
	wait.until(ExpectedConditions.visibilityOfAllElementsLocatedBy(element));
	if (driver.findElement(element).isDisplayed()) {
		report.updateTestLog("Open " + pagename, pagename+" opened successfully", Status.PASS);
		System.out.println("Open "+pagename);
	}else{
		report.updateTestLog("Open "+pagename, pagename+" not opened", Status.FAIL);			
...
public void Mousehover(By element){
	...
	Actions action=new Actions(driver.getWebDriver());
	action.moveToElement(driver.findElement(element)).build().perform();
	report.updateTestLog("Mouse over the element", "Mouseover is successful", Status.PASS);
	System.out.println("Mouseover is successful");
...
public void SelectDropdownByIndex(By element, int value){
...
public void Scrollview( By OR,String Operation) {
	try{
		...
		report.updateTestLog("Scrollview ", "Scroll view  error "+e, Status.FAIL);
	}
...
public void mfaemail(By element, String name) {
    try {
    		//MFA  Page handling
    		String Jenkins = properties.getProperty("JenkinsCred"); // Getting the property to confirm jenkins run
    	 	Thread.sleep(3000);
    	 	if (Jenkins.equalsIgnoreCase("true")){
    		isDisplayed(OutlookOR.rdo_email, "email");
			waitFOR(OutlookOR.rdo_email);
			driver.findElement(OutlookOR.rdo_email).click();
    	 	}else {
    	 		isDisplayed(OutlookOR.rdo_emaillocal, "email");
    			waitFOR(OutlookOR.rdo_emaillocal);
    			driver.findElement(OutlookOR.rdo_emaillocal).click();
    	 	}
			Thread.sleep(3000);
            waitFOR(OutlookOR.btn_next);
            Javascript_Click_Scroll(OutlookOR.btn_next, "Scroll");
            Thread.sleep(3000);
            driver.findElement(OutlookOR.btn_next).click();
            Thread.sleep(5000);
            loginOutlook();  
            Thread.sleep(3000);
            Javascript_Click_Scroll(OutlookOR.txt_verification, "Scroll");
            driver.findElement(OutlookOR.txt_verification).click();
            SendKeys(OutlookOR.txt_verification, dataTable.getData("General_Data", "OTP"));
            driver.findElement(OutlookOR.btn_verify).click();
            waitFOR(element);
            if(isDisplayed_boolean(element,name)) {
                  report.updateTestLog("Login",name+" is Displayed", Status.PASS);
            }else{
                  report.updateTestLog("Login",name+" is NOT Displayed", Status.FAIL);
            }
			
             
    }catch(Exception ex) {
          report.updateTestLog("Login","Unable to login the application", Status.FAIL);
    }
}
...
public String loginOutlook(){
	...
	String Jenkins = properties.getProperty("JenkinsCred"); // Getting the property to confirm jenkins run
	WebDriver driver;
	//Intializing chrome options for Incognito mode and outlook launch
    ...
    Ioptions.addArguments("enable-automation");
    Ioptions.addArguments("--window-size=1920,1080");
    Ioptions.addArguments("--no-sandbox");
    Ioptions.addArguments("--disable-extensions");
    Ioptions.addArguments("--dns-prefetch-disable");
    Ioptions.addArguments("--disable-gpu");

    driver = new ChromeDriver(Ioptions);
    WebDriverWait wait = new WebDriverWait (driver,30);
    //Getting Sign in data from datatable
	String Subject = dataTable.getData("General_Data", "subject");
	String EmailID = dataTable.getData("General_Data", "EmailID");
	String emailOtp = dataTable.getData("General_Data", "OTP");	
	String EmailPwd = dataTable.getData("General_Data", "EmailPassword");	

	try{	
		OutlookOR or=new OutlookOR();	
			
        driver.get(dataTable.getData("General_Data", "outURL")); //Launching the Url
    
        Thread.sleep(15000);
        
        //Sign in with credentials for Jenknis Run
        if (Jenkins.equalsIgnoreCase("true")){	
        if (driver.findElement(or.txt_Email).isDisplayed()) {
        	System.out.println("Sign_email xpath validated");
        	
           	 	driver.findElement(or.txt_Email).sendKeys(EmailID);
                driver.findElement(or.btn_Next).click();
                
                Thread.sleep(8000);
                driver.navigate().refresh();
                wait.until(ExpectedConditions.presenceOfElementLocated(or.lbl_EmailAddress));
                
                driver.findElement(or.txt_username).sendKeys(EmailID);
                driver.findElement(or.txt_password).sendKeys(EmailPwd);
                driver.findElement(or.btn_SignOn).click();
                Thread.sleep(5000);
        	
             if(driver.findElement(or.lbl_StaySigned).isDisplayed())
             {
            	 driver.findElement(or.btn_StaySignedNo).click();
            	 System.out.println("The Stay signed in page is  displayed in Outlook, Selected No.");
             }
             else {
            	 System.out.println("The Stay signed in page is not displayed in Outlook, So continuing the script.");
             }
             }
        }
        else{
        	//Waiting for outlook label
        	dowait(or.lbl_Outlook, "Outlook label", 5);
        	isDisplayed_boolean(or.lbl_Outlook, "Outlook Application launched without Sign in");
		}
        
//        else{
//        	driver.findElement(or.btn_user).click();
//		}
        
        wait.until(ExpectedConditions.presenceOfElementLocated(or.lbl_Outlook));
        wait.until(ExpectedConditions.visibilityOfElementLocated(or.lbl_Outlook));
      	System.out.print("OutLook application launched");
		...
		Thread.sleep(3000);
		// wait.until(ExpectedConditions.presenceOfElementLocated(or.lbl_Outlook));
		report.updateTestLog("Outlook launch","Outlook opened successfully", Status.PASS);
		
	}catch(Exception e){
    	System.out.println("Exception in login: "+e.toString());
    	report.updateTestLog("Outlook launch","Outlook not launched"+e, Status.FAIL);
    }
	
	//Gets the MFA code from a folder inside outlook for a Local run
	try {
		if (Jenkins.equalsIgnoreCase("false")){		
			JavascriptExecutor js=(JavascriptExecutor)driver; 
			String FolderName = dataTable.getData("General_Data", "EmailFolder");
			js.executeScript("arguments[0].scrollIntoView(true);",driver.findElement(By.xpath("//div[contains(@title,'"+FolderName+"')]")));
			Thread.sleep(3000);
			driver.findElement(By.xpath("//div[contains(@title,'"+FolderName+"')]")).click();
			Thread.sleep(4000);				
			List<WebElement> msgsize=driver.findElements(By.xpath("//div[@role='option']/div/div"));
			System.out.print(msgsize.size());
			Thread.sleep(4000);
			for(int i=1;i<=msgsize.size();){
				String msgtxt = driver.findElement(By.xpath("(//div[@role='option'])["+i+"]")).getAttribute("aria-label");
				if(msgtxt.contains(Subject)){
					 report.updateTestLog("Mail Validation","Email received successfully", Status.PASS);
					 driver.findElement(By.xpath("(//div[@role='option'])["+i+"]")).click();
					 Thread.sleep(13000); 
				     emailOtp = driver.findElement(By.xpath("//*[@class='x_container']//div[2]/b")).getText();
				     Thread.sleep(13000);
				     System.out.println("----OTP------"+emailOtp);	
				     dataTable.putData("General_Data", "OTP", emailOtp); 
				     driver.close();
					 break;
				}else{
					i++;
			...
			//Gets the MFA code from the email, no folders involved. This is used for jenkins run
			...
			for(int i=1;i<=msgsize.size();){
				...
				if(msgtxt.contains(Subject)){
				     ...
				     dataTable.putData("General_Data", "OTP", emailOtp);
				     driver.close();
				   //  String parentwindow1  = window_handle();			
				     /*driver.findElement(OutlookOR.btn_username).click();
				     driver.findElement(OutlookOR.btn_Signout).click();*/
//				     driver.switchTo().window(Windows.get(0));		
		...
		System.out.print("Exception in clicking the link "+e.getMessage());				
	}		
	return emailOtp;
} 
